...
for name, clf in classifiers.items():
    ...
    if args.pca_components is not None:
        cm_filename = os.path.join(graphics_dir, f"confusion_matrix_{name.replace(' ', '_').lower()}_pca{n_pca}_cifar10.png")
    else:
        cm_filename = os.path.join(graphics_dir, f"confusion_matrix_{name.replace(' ', '_').lower()}_sem_pca_cifar10.png")
...
plt.ylabel("Acurácia")
plt.title(f"Comparação de Acurácia dos Classificadores (CIFAR-10)\n{pca_info}")
...
if args.pca_components is not None:
    output_file = os.path.join(graphics_dir, f"cifar10_classifiers_comparison_pca{n_pca}.png")
else:
    output_file = os.path.join(graphics_dir, "cifar10_classifiers_comparison_sem_pca.png")
