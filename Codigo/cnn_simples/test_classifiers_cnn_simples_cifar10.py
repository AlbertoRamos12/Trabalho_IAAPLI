...
# ----------------------------
parser = argparse.ArgumentParser(description="Testar classificadores no CIFAR-10 usando deep features extraídas de uma CNN.")
...
# Remover pesos da última camada se existirem
...
if args.pca_components is not None:
    ...
    # Descobrir número real de componentes usados (útil se n_components for float)
...
for name, clf in classifiers.items():
    ...
    if args.pca_components is not None:
        cm_filename = os.path.join(graphics_dir, f"cnn_simples_confusion_matrix_{name.replace(' ', '_').lower()}_pca{n_pca}_cifar10.png")
    else:
        cm_filename = os.path.join(graphics_dir, f"cnn_simples_confusion_matrix_{name.replace(' ', '_').lower()}_sem_pca_cifar10.png")
...
plt.ylabel("Accuracy")
plt.title(f"Comparação de Accuracy dos Classificadores (CIFAR-10)\n{pca_info}")
...
plt.title(f"Tempo de Treinamento dos Classificadores (CIFAR-10)\n{pca_info}")
plt.xticks(rotation=45)

plt.tight_layout()

# Nome do ficheiro inclui info do PCA
if args.pca_components is not None:
    output_file = os.path.join(graphics_dir, f"cnn_simples_cifar10_classifiers_comparison_pca{n_pca}.png")
else:
    output_file = os.path.join(graphics_dir, "cnn_simples_cifar10_classifiers_comparison_sem_pca.png")
